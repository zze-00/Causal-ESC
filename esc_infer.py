...
from esc_dataset import BaseDataset
...
parser.add_argument("--load_checkpoint", '-c', type=str, default= "/data/zhuoer/ESC/simple_graph/my_result/2024-04-28054829.3e-05.8.1gpu/step-2800.bin")
...
testset = BaseDataset(dataset_type='test',toker=toker)
...
for batch,responses,sample_ids in tqdm.tqdm(infer_loader, total=len(infer_loader),desc="Inferring"):
    ...
    for idx in range(len(batch['cls_indices'])):
        ...
        if not args.only_encode:   # True
            ...
            tmp_res_to_append = {'sample_id': sample_ids[idx], 'post': p, 'response': r, 'generation': g}    # 记录
            #print('> context:   ', p)
            #print('> generation:', g)
        else:    # 不用管
